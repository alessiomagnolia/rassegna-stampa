...
async function extractLogo(url, sourceName = '') {
    try {
        ...
        // Try extracting from homepage HTML with a quick 3.5s timeout (never hangs)
        try {
            ...
            console.log(`[Logo Extractor] Avviso: scansione diretta fallita per ${originUrl} (${fetchErr.message}), passo ai servizi logo.`);
        }

        // Resolve absolute URL if found on homepage
        ...
        // 3. Fallback 1: Clearbit Brand Logo API
        if (!finalLogoBase64) {
            try {
                finalLogoBase64 = await downloadImageAsBase64(`https://logo.clearbit.com/${domainHost}`);
            } catch(e){}
        }

        // 4. Fallback 2: Google S2 High-Res 128px Favicon / Brand Icon
        if (!finalLogoBase64) {
            try {
